...
def statistical_feature_values(compound,feature_collection,feature_name,statistical_functions='all'):
    ...
    # tmp_comp = Composition(compound)
    # tmp_comp_dict = tmp_comp.as_dict()
    # atoms      = list(tmp_comp_dict.keys())
    # subscripts = list(tmp_comp_dict.values())

    atoms, subscripts = building_blocks(compound)
    # atoms = np.repeat(elements, subscripts)
    stats_fea=[]
    for atom in atoms:
        tmp_val = feature_collection.loc[feature_collection['chemical_symbol'] == atom][feature_name].values
        stats_fea.append(tmp_val[0])

    stats_fea_vals = np.repeat(stats_fea, subscripts)
...
def fetch_wstatistical_features(compounds, collection, features='all', column_name=None, statistical_functions='all'):
